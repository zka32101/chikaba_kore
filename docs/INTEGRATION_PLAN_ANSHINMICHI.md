...
- [x] 地図タブ内に「お店を探す」「安全ルートを探す」のモード切り替えを実装（サブフェーズ3a〜3d完了。周辺機能（投稿確認・コメント・塗って投稿・本人確認）も実装済み）
- ブランドを統一（名称は要検討）
- 各アプリの独自機能はモードとして共存
- 残作業: Cloud Functions移植（サブフェーズ3e/3f着手前に必要。`docs/PHASE3_MODE_INTEGRATION_DESIGN.md`の「次のアクション」参照）、project-039リポジトリのアーカイブ判断（サブフェーズ3f）
